...
previous_bid_period <- (as.character(format(seq(Sys.Date(), by = "-3 month", length = 2)[2], "%Y-%m"))) 

previous_filt_bid_period <- (as.character(format(seq(Sys.Date(), by = "-2 month", length = 2)[2], "%Y-%m"))) 
...
combined_gold <- inner_join(clean_fp, clean_reserve) %>% 
  ...
  pivot_wider(names_from = BASE, values_from = workload, names_prefix = "workload")
